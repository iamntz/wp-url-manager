...
add_action('admin_enqueue_scripts', function () {
	wp_register_script('ntz-url-manager', plugins_url('/url-manager.js', __FILE__), ['jquery', 'jquery-ui-autocomplete'], NTZ_URL_MANAGER_VERSION, true);

	wp_enqueue_style('ntz-url-manager-ui', 'https://code.jquery.com/ui/1.12.0/themes/smoothness/jquery-ui.css', [], NTZ_URL_MANAGER_VERSION, 'screen');
	wp_enqueue_style('ntz-url-manager', plugins_url('/url-manager.css', __FILE__), ['ntz-url-manager-ui'], NTZ_URL_MANAGER_VERSION, 'screen');
...
add_action('wp_ajax_ntz_url_manager_search', function () {
  ...
  $results = $wpdb->get_results($wpdb->prepare("SELECT * FROM {$wpdb->prefix}url_manager WHERE url LIKE %s OR title LIKE %s  ORDER BY hits DESC, id DESC LIMIT 20", $url, $url));
